...
import {scrollToService} from "@/lib/utils";
...
export default function ServiceHero() {
    return (
        <div className="bg-linear-to-b from-green-300 via-green-300 to-green-50 mx-auto px-4 py-20 sm:py-26">
            ...
            <div className="mt-12 lg:mt-16 mx-auto max-w-2xl grid grid-cols-1 gap-8 lg:gap-x-16 lg:max-w-7xl lg:grid-cols-2">
                {services.map((service, index) => (
                    <Link key={service.name} href={`/services/#${service.id}`} onClick={() => scrollToService(service.id)} style={{animationDelay: `${index * 200}ms`, animationFillMode: "forwards"}} className="bg-green-100 group flex gap-x-4 items-center p-4 rounded-xl border border-gray-900 shadow-md hover:shadow-xl transition duration-300 hover:-translate-y-1 animate-fade-up opacity-0">
                        <div className="flex items-center justify-center size-12 rounded-xl bg-green-200 text-gray-900 group-hover:bg-emerald-600 group-hover:text-white transition duration-300">
